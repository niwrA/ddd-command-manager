...
namespace CommandManagerCoreTests
{
    ...
    public class IntegrationTests
    {
        ...
        public void ProcessCommand_CanTargetMultipleProcessorsForOneCommand()
        {
            ...
            var commandDto = new CommandDto { Entity = "RootEntity", Command = "Create", EntityGuid = Guid.NewGuid(), Guid = Guid.NewGuid(), CreatedOn = new DateTime(2018, 1, 1), ParametersJson = @"{Name: 'James Smith'}" };
        ...
        public void CommandManager_CanMergeCommandsWithExistingStateAndApply()
        {
            ...
            // todo: make builder
            var createCommandDto = new CommandDto { CreatedOn = DateTime.Now.AddTicks(1), Entity = "RootEntity", EntityGuid = entityGuid, Guid = Guid.NewGuid(), Command = "Create", ParametersJson = @"{Name: 'John Smith'}" };
            var renameCommandDto = new CommandDto { CreatedOn = DateTime.Now.AddTicks(1), Entity = "RootEntity", EntityGuid = entityGuid, Guid = Guid.NewGuid(), Command = "Rename", ParametersJson = @"{Name: 'James Smith'}" };
            ...
            // merge the commands
            var sut2 = new CommandService(repo.Object, dateTimeProvider);
            sut2.MergeCommands(typedCommands);
